...
# [1, 0, 2, 0, 0, 0]
# (0,9), (0,8), (0,7)...(0,0)
# a cada alocação vai diminuir 1 espaço disponivel 
...
def funcao_custo(solucao):
    ...
    for i in range(len(solucao)):
        atual = solucao[i]
        dormitorio = dormitorios[vagas[atual]]
        # seleciona qual dormitorio a pessoa sera associada
        preferencia = preferencias[i][1]
        if preferencia[0] == dormitorio:
            # se a preferencia 1 for igual o dormitorio atribuido
            custo += 0
        elif preferencia[1] == dormitorio:
            # se a preferencia 2 for igual o dormitorio atribuido
            custo += 1
        else:
            # se a preferencia nao for nenhuma das selecionadas
            custo += 3
        
        del vagas[atual]
        # apaga a vaga que foi preenchida
